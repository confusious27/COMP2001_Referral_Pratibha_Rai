...
/*Data*/
INSERT INTO CW1.[Trail] (TrailName, Rating, Difficulty, Location, Distance, EstimatedTime, ElevationGain, TrailType, Description) VALUES
...
/*Data*/
INSERT INTO CW1.[User] (Email, Role) VALUES
...
Trail_ID INT FOREIGN KEY REFERENCES CW1.[Trail](Trail_ID),
User_ID INT FOREIGN KEY REFERENCES CW1.[User](User_ID),
Content VARCHAR(500),
CreatedOn DATETIME,
IsArchived BIT
);
...
/*Data*/
INSERT INTO CW1.[Comment] (Trail_ID, User_ID, Content, CreatedOn, IsArchived) VALUES
...
SELECT * FROM CW1.Comment;



/*Trail Details with Comments View*/
CREATE VIEW CW1.TrailWithCommentsView AS
SELECT
    T.Trail_ID,
    T.TrailName,
    T.Rating,
    ...
    C.IsArchived
FROM CW1.[Trail] T
LEFT JOIN CW1.[Comment] C ON T.Trail_ID = C.Trail_ID AND C.IsArchived = 0 /*to ensure that only unarchived comments can be seen*/
LEFT JOIN CW1.[User] U ON C.USER_ID = U.USER_ID;

SELECT * FROM CW1.TrailWithCommentsView;



--CREATE
CREATE PROCEDURE CW1.InsertComment
    @Trail_ID INT,
    @User_ID INT,
    @Content VARCHAR(500),
    @CreatedOn DATETIME,
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM CW1.[User] WHERE USER_ID = @User_ID)
        AND EXISTS (SELECT 1 FROM CW1.[Trail] WHERE @Trail_ID = @Trail_ID)
    BEGIN
        INSERT INTO CW1.[Comment] (Trail_ID, User_ID, Content, CreatedOn, IsArchived)
...
/*Data*/
EXEC CW1.InsertComment '3', '2', 'Make sure to research thoroughly and familiarise yourself with the route before you attempt.', '2024-12-04 23:04:49', '0';


--READ
CREATE PROCEDURE CW1.GetActiveComments
AS
BEGIN
...
EXEC CW1.GetActiveComments;


--UPDATE
CREATE PROCEDURE CW1.UpdateComment
    @Comment_ID INT,
    @User_ID INT,
    @Content VARCHAR(500),
    @IsArchived BIT
...
EXEC CW1.UpdateComment '2', '3', 'Nice and easy. Good for an evening stroll.', '0';


--DELETE (Archive)
CREATE PROCEDURE CW1.ArchiveComment(@Comment_ID INT, @User_ID INT)
AS
BEGIN
...
--non-admin
EXEC CW1.ArchiveComment '2', '2';
--admin
EXEC CW1.ArchiveComment '2', '1';


/*Trigger Exercise*/
CREATE TABLE CW1.AddNewTrail (
...
ADD AddedBy INT FOREIGN KEY REFERENCES CW1.[User](User_ID);
...
BEGIN
    ...
    INSERT INTO CW1.AddNewTrail (Trail_ID, User_ID, CreatedOn)
...
--testing
INSERT INTO CW1.Trail (TrailName, Rating, Difficulty, Location, Distance, EstimatedTime, ElevationGain, TrailType, Description, AddedBy) VALUES
